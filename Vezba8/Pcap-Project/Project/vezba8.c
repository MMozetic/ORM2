...
int main()
{
	...
	if (arp_dumper == NULL)
	{
		printf("\n Error opening arp output file\n");
		return -1;
	}

	if (icmp_dumper == NULL)
	{
		printf("\n Error opening icmp output file\n");
		return -1;
	}

	if (udp_dumper == NULL)
	{
		printf("\n Error opening udp output file\n");
		return -1;
	}

	if (tcp_dumper == NULL)
	{
		printf("\n Error opening tcp output file\n");
		return -1;
	}
	...
	// Compile the filter
	if (pcap_compile(device_handle, &fcode, "arp or udp or icmp or tcp", 1, 0xffffff) < 0)
	{
